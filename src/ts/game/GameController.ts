var GameTeste = function()
{
    ...
    this.setControlPos = function(pos)
    {
        ...
        if(this.ship.pos.y < 8){
            this.ship.pos.y = 8;
        }
        else if( this.ship.pos.y > (self.getHeight() - self.ship.canvas.offsetHeight)){
            this.ship.pos.y = self.getHeight() - self.ship.canvas.offsetHeight;
        }
    ...
    {
        $.ajax({ type: 'GET', url: 'ranking', dataType: "json" }).done(function(data){ self.showScore(data); });
    }
    ...
    this.update = function()
    {
        ...
        if(self.keystatus.space || self.keystatus.up || self.keystatus.down)
        {
            if(!self.isGameOver)
            {
                ...
                if(self.ship.pos.y < 8){
                    self.ship.pos.y = 8;
                }else if(self.ship.pos.y > (self.getHeight() - self.ship.canvas.offsetHeight)){
                    self.ship.pos.y = self.getHeight() - self.ship.canvas.offsetHeight;
                }
        ...
        if(window.requestAnimationFrame) window.requestAnimationFrame(function(){self.update();});
        else if(window.requestAnimationFrame) window.webkitRequestAnimationFrame(function(){self.update();});
        else if(window.requestAnimationFrame) window.mozRequestAnimationFrame(function(){self.update();});
        else if(window.requestAnimationFrame) window.oRequestAnimationFrame(function(){self.update();});
        else if(window.requestAnimationFrame) window.msRequestAnimationFrame(function(){self.update();});
        else window.setTimeout(function(){self.update();}, 1000/60);
    }
